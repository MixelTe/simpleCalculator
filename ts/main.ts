...
{
	if (!state.clearInputOnType) calc();
...
function unary(calcF: (x: number) => number)
{
...
function calc()
{
	if (state.saved == null || state.operation == "") return;
	if (state.saved2 != null && state.clearInputOnType)
		state.saved = state.input;
	else
...
function updateUI()
{
	if (state.saved === null) stateEl.innerText = "";
